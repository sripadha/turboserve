...
[`deploy/prometheus/rules.yaml`](../deploy/prometheus/rules.yaml) is one file with two
consumers: a standalone Prometheus loads it through `rule_files`, and the chart embeds it
verbatim in a `PrometheusRule` (`deploy/helm/turboserve/files/prometheus-rules.yaml` is a
symlink to it, and Helm follows symlinks inside a chart when it loads or packages one). The
alerts in a cluster and the alerts on a laptop therefore cannot drift apart.
...
- The development machine for this repository has no Docker (WSL without Docker Desktop
  integration) and a 6 GB Turing GPU, so **nothing here has been applied to a cluster from
  this checkout**. The manifests are validated statically with `helm lint` and kubeconform,
  and end-to-end on GitHub's runners by the kind workflow.
